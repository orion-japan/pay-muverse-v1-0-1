...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { user_code, title, body, url, tag, image, icon, badge, renotify, vibration } =
      input || {};

    if (!user_code) {
      return new Response(JSON.stringify({ ok: false, reason: 'missing user_code' }), {
        status: 400,
      });
    }
    ...
    if (!Array.isArray(subs) || subs.length === 0) {
      return new Response(JSON.stringify({ ok: false, reason: 'no subscription for user_code' }), {
        status: 404,
      });
    }
