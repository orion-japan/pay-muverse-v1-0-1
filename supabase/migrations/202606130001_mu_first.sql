...
create index if not exists idx_user_journey_events_user_time
on public.user_journey_events (user_code, occurred_at desc);

create index if not exists idx_mu_screenshot_diagnosis_logs_user_time
...
as $$
begin
  insert into public.mu_screenshot_credit_grants (user_code, amount, reason, campaign)
  values (p_user_code, p_amount, p_reason, p_campaign)
