...
function pickQ(
...
): { q: AinoriQResult['q']; confidence: number; hint: string } {
  ...
  // 欠席から復帰した当日は転機扱い
  if (isPresentToday && m.streakAbsent >= 2)
    return { q: 'Q5', confidence: 0.7, hint: '欠席明けの復帰' };
...
function streak(dates: string[], presentSet: Set<string>, type: 'attend' | 'absent'): number {
  ...
  for (let i = dates.length - 1; i >= 0; i--) {
    ...
    if ((type === 'attend' && attend) || (type === 'absent' && !attend)) s++;
    else break;
  }
  return s;
}

export async function calcAinoriQForDate(
  user_code: string,
  forDateIso: string,
): Promise<AinoriQResult> {
