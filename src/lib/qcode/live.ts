 ...
 */
export async function calcLiveQForDate(
  user_code: string,
  forDateIso: string,
): Promise<LiveQResult> {
  ...
  const activity = +(0.45 * dens14 + 0.25 * dens30 + 0.2 * recency + 0.1 * rhythm).toFixed(4);
  ...
  const confidence =
    q === 'Q2' ? 0.72 : q === 'Q1' ? 0.65 : q === 'Q3' ? 0.6 : q === 'Q4' ? 0.7 : 0.7;

  const hint = `14d:${joins14Count}回 / 30d:${joins30Count}回 / last:${lastJoinDaysAgo ?? '—'}日 / act:${activity}`;
