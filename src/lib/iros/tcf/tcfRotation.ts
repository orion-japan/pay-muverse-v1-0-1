...
function normalizeTcfCDirection(value: unknown): TcfCDirection | null {
  const v = firstString(value)?.toLowerCase() ?? null;
  if (
    v === 'none' ||
    v === 'concretize' ||
    v === 'structure_design' ||
...
function normalizeTcfUserReaction(value: unknown): TcfUserReaction | null {
  const v = firstString(value)?.toLowerCase() ?? null;
  if (
    v === 'accept' ||
    v === 'reject' ||
...
function normalizeTcfConvergence(value: unknown): TcfConvergenceState | null {
  const v = firstString(value)?.toLowerCase() ?? null;
  if (
    v === 'none' ||
    v === 'focused' ||
...
function normalizeTcfAnchorEvent(value: unknown): TcfAnchorEvent | null {
  ...
  if (event === 'choice' || event === 'action' || event === 'reconfirm' || event === 'none') return event;
  return null;
}
...
function buildTcfTEvidenceReason(args: {
  ...
  if (args.anchorEventRaw && args.anchorEventRaw !== 'none') return `ANCHOR_EVENT_${args.anchorEventRaw.toUpperCase()}`;
  return null;
}
...
export function readTcfTEvidence(input: TcfTEvidenceInput): TcfTEvidence {
  ...
  const ctxPack = asRecord(input.ctxPack) ?? asRecord(extra.ctxPack) ?? asRecord(meta.ctxPack) ?? {};
  const sriContext = asRecord(input.sriContext) ?? asRecord(ctxPack.sriContext) ?? asRecord(extra.sriContext) ?? asRecord(meta.sriContext) ?? {};
  const intentionContext = asRecord(sriContext.intentionContext) ?? asRecord(sriContext.intentContext) ?? {};
  const memoryState = asRecord(input.memoryState) ?? asRecord(meta.memoryState) ?? asRecord(extra.memoryState) ?? asRecord(ctxPack.memoryState) ?? {};
  const anchorEntry = asRecord(input.anchorEntry) ?? asRecord(meta.anchorEntry) ?? asRecord(extra.anchorEntry) ?? asRecord(ctxPack.anchorEntry) ?? {};
  ...
  const hasT = Boolean(itxStep === 'T3' || itxStep === 'T2' || tEntryOk || hasCommittedAnchor || hasAnchorEvent);
  ...
  return {
    ...
    anchorEvent,
    hasCommittedAnchor,
    reason: buildTcfTEvidenceReason({ itxStep, tEntryOk, hasCommittedAnchor, anchorEventRaw }),
  };
}
...
export function resolveTcfCDirection(input: ResolveTcfCDirectionInput): TcfCDirection {
  ...
  const sriContext = asRecord(input.sriContext) ?? asRecord(ctxPack.sriContext) ?? asRecord(extra.sriContext) ?? asRecord(meta.sriContext) ?? {};
  const intentionContext = asRecord(sriContext.intentionContext) ?? asRecord(sriContext.intentContext) ?? {};
  const focusResolution = asRecord(input.focusResolution) ?? asRecord(ctxPack.focusResolution) ?? asRecord(extra.focusResolution) ?? asRecord(meta.focusResolution) ?? {};
  const transferSeed = input.transferSeed ?? ctxPack.transferSeed ?? extra.transferSeed ?? meta.transferSeed ?? null;
  const memoryIntent = firstString(input.memoryIntent, ctxPack.memoryIntent, extra.memoryIntent, meta.memoryIntent, intentionContext.memoryIntent)?.toLowerCase() ?? null;
  ...
  if (hasTcfMatch(text, [/違う/u, /そうじゃない/u, /ズレ/u, /戻ってる/u, /もう一回/u, /言い方/u, /硬い/u])) return 'writer_correction';
  if (hasTcfMatch(text, [/実装/u, /コード/u, /関数/u, /db/u, /sql/u, /保存/u, /取得/u, /型/u, /ファイル/u, /import/u])) return 'implementation';
  if (memoryIntent === 'diagnosis_recall' || hasTcfMatch(text, [/診断/u, /深め/u, /状態/u, /ir/u, /カード/u])) return 'diagnosis_deepen';
  if (memoryIntent === 'relationship_recall' || hasTcfMatch(text, [/関係/u, /距離感/u, /連絡/u, /相手/u, /境界線/u, /どう返す/u, /離れる/u, /近づく/u])) return 'relation_boundary';
  if (memoryIntent === 'working_rule_recall' || memoryIntent === 'project_context_recall' || memoryIntent === 'past_context_recall' || hasTcfMatch(text, [/覚えて/u, /前の話/u, /記憶/u, /呼び出し/u, /memory/u, /pre-seed/u])) return 'memory_seed';
  if (goalKind === 'decide' || hasTcfMatch(text, [/やる/u, /進め/u, /次/u, /手順/u, /どう動く/u, /何から/u, /お願いします/u])) return 'action_plan';
  if (hasTcfMatch(text, [/構造/u, /設計/u, /仕様/u, /仕組み/u, /全体像/u, /回路/u, /接続/u, /整理/u, /tcf/u, /sri/u, /focus/u])) return 'structure_design';
  if (hasTcfMatch(text, [/具体/u, /形に/u, /落とす/u, /判断基準/u, /扱い方/u])) return 'concretize';
  return 'none';
}
...
export function detectTcfUserReaction(userText: string | null | undefined): TcfUserReaction {
  ...
  if (hasTcfMatch(text, [/違う/u, /そうじゃない/u, /ズレ/u, /外れ/u, /戻ってる/u, /それは違う/u, /そこじゃない/u, /もう言ってる/u, /深読みしない/u])) return 'reject';
  if (hasTcfMatch(text, [/どういうこと/u, /よく分から/u, /よくわから/u, /わからない/u, /分からない/u, /まだ見えない/u, /どこから/u, /混乱/u, /不明/u])) return 'confused';
  if (hasTcfMatch(text, [/だいたい/u, /大体/u, /近い/u, /方向は合/u, /方向性は合/u, /もう少し/u, /でも/u, /ただ/u, /少し違/u, /調整/u, /修正/u, /言い換/u])) return 'refine';
  if (hasTcfMatch(text, [/詳しく/u, /もう少し説明/u, /教えて/u, /どうすれば/u, /どうやって/u, /どれ/u, /何を/u, /なにを/u, /どこ/u, /ありますか/u, /できますか/u, /\?$/, /？$/])) return 'ask_more';
  if (hasTcfMatch(text, [/やります/u, /進めます/u, /進めましょう/u, /やろう/u, /作って/u, /入れて/u, /追加/u, /実装/u, /保存して/u, /コミット/u, /push/u, /プッシュ/u, /お願いします/u, /お願い/u])) return 'action';
  if (hasTcfMatch(text, [/それでいい/u, /これでいい/u, /それです/u, /そういうこと/u, /合ってる/u, /合っています/u, /ok/u, /ＯＫ/u, /了解/u, /承知/u, /いいです/u, /大丈夫/u, /ありがとう/u, /ありがとうございます/u])) return 'accept';
  return 'unknown';
}
...
export function decideTcfConvergence(input: DecideTcfConvergenceInput): TcfConvergenceState {
  ...
  if (cDirection === 'writer_correction' || userReaction === 'reject') return 'diverged';
  if (hasDifferentTcfFocus(previousFocus, currentFocus) && (userReaction === 'refine' || userReaction === 'action' || userReaction === 'accept')) return 'recycle';
  if (userReaction === 'refine') return 'partial';
  if (userReaction === 'confused') return 'unresolved';
  if (userReaction === 'ask_more') return hasFocus ? 'partial' : 'unresolved';
  if (userReaction === 'accept') return hasFocus || hasT || hasCDirection ? 'converged' : 'focused';
  if (userReaction === 'action') return hasT || hasCDirection || hasFocus ? 'converged' : 'focused';
  if (hasT && hasCDirection && hasFocus) return 'focused';
...
function resolveTcfWriterPatternKey(args: { cDirection: TcfCDirection; convergence: TcfConvergenceState }): string | null {
  ...
  if (args.cDirection === 'implementation') return 'TCF_IMPLEMENTATION_V1';
  if (args.cDirection === 'memory_seed') return 'SEED_DESIGN_V1';
  if (args.cDirection === 'structure_design') return 'STRUCTURE_DESIGN_V1';
  return null;
}
...
function resolveTcfSurfacePlanKind(args: { cDirection: TcfCDirection; convergence: TcfConvergenceState }): string | null {
  ...
  if (args.cDirection === 'implementation') return 'implementation';
  if (args.cDirection === 'structure_design' || args.cDirection === 'concretize' || args.cDirection === 'memory_seed') return 'structure_design';
  return null;
}

function shouldPersistTcfFocus(args: { convergence: TcfConvergenceState; tEvidence: TcfTEvidence }): boolean {
  if (args.convergence === 'diverged' || args.convergence === 'unresolved') return false;
  if (args.convergence === 'converged' || args.convergence === 'partial' || args.convergence === 'focused') return true;
  return args.tEvidence.hasT === true;
}

function shouldRebuildTcfFocus(convergence: TcfConvergenceState): boolean {
  return convergence === 'partial' || convergence === 'unresolved' || convergence === 'diverged' || convergence === 'recycle';
}
...
function buildTcfRotationReason(args: {
  ...
  userReaction: TcfUserReaction;
  convergence: TcfConvergenceState;
  preSeedStarterApplied: boolean;
}): string {
  return [
    args.preSeedStarterApplied ? 'preseed_tcf_starter=applied' : 'preseed_tcf_starter=none',
...
function readPreSeedTcfStarter(input: BuildTcfRotationDecisionInput): Record<string, any> | null {
  const meta = asRecord(input.meta) ?? {};
  const extra = asRecord(input.extra) ?? asRecord(meta.extra) ?? {};
  const ctxPack = asRecord(input.ctxPack) ?? asRecord(extra.ctxPack) ?? asRecord(meta.ctxPack) ?? {};
  const nestedExtra = asRecord(meta.extra) ?? {};
  const nestedExtraCtxPack = asRecord(nestedExtra.ctxPack) ?? {};

  const candidates = [
    asRecord((input as any).tcfStarter),
    asRecord(ctxPack.tcfStarter),
    asRecord(extra.tcfStarter),
    asRecord(meta.tcfStarter),
    asRecord(nestedExtra.tcfStarter),
    asRecord(nestedExtraCtxPack.tcfStarter),
  ];

  return candidates.find((candidate) => candidate && Object.keys(candidate).length > 0) ?? null;
}

export function buildTcfRotationDecision(input: BuildTcfRotationDecisionInput): TcfRotationDecision {
  const meta = asRecord(input.meta) ?? {};
  const extra = asRecord(input.extra) ?? asRecord(meta.extra) ?? {};
  const ctxPack = asRecord(input.ctxPack) ?? asRecord(extra.ctxPack) ?? asRecord(meta.ctxPack) ?? {};
  const focusResolution = asRecord(input.focusResolution) ?? asRecord(ctxPack.focusResolution) ?? asRecord(extra.focusResolution) ?? asRecord(meta.focusResolution) ?? {};
  const transferSeed = input.transferSeed ?? ctxPack.transferSeed ?? extra.transferSeed ?? meta.transferSeed ?? null;
  const starter = readPreSeedTcfStarter(input);
  ...
  const hasMemoryRecallNotFoundTurnContract = (() => {
    ...
    return contracts.some((contract) => {
      ...
      if (!c) return false;
      return String(c.turnTask ?? '').trim() === 'memory_recall_check' &&
  ...
  const tEvidence = input.tEvidence ?? readTcfTEvidence({
    meta,
    extra,
    ctxPack,
  ...
  const starterCDirection = normalizeTcfCDirection(starter?.cDirection ?? starter?.c_direction);
  const starterUserReaction = normalizeTcfUserReaction(starter?.userReaction ?? starter?.user_reaction);
  const starterConvergence = normalizeTcfConvergence(starter?.convergence);

  const cDirection = hasMemoryRecallNotFoundTurnContract
    ? 'none'
    : input.cDirection ?? starterCDirection ?? resolveTcfCDirection({
  ...
  const userReaction = hasMemoryRecallNotFoundTurnContract
    ? 'unknown'
    : input.userReaction ?? starterUserReaction ?? detectTcfUserReaction(input.userText);

  const convergence = hasMemoryRecallNotFoundTurnContract
    ? 'none'
    : input.convergence ?? starterConvergence ?? decideTcfConvergence({
        previousFocus,
        currentFocus,
        userReaction,
        tEvidence,
  ...
  const shouldPersistFocus = hasMemoryRecallNotFoundTurnContract ? false : shouldPersistTcfFocus({ convergence, tEvidence });
  const shouldRebuildFocus = hasMemoryRecallNotFoundTurnContract ? false : shouldRebuildTcfFocus(convergence);
  const shouldPromoteDepth = !hasMemoryRecallNotFoundTurnContract && (convergence === 'converged' || (tEvidence.hasT && cDirection !== 'none'));
  const shouldRouteToC = !hasMemoryRecallNotFoundTurnContract && cDirection !== 'none' && convergence !== 'diverged' && convergence !== 'unresolved';
  ...
  return {
    ...
    reason: hasMemoryRecallNotFoundTurnContract
      ...
      : buildTcfRotationReason({
          ...
          userReaction,
          convergence,
          preSeedStarterApplied: Boolean(starter),
        }),
  };
}
...
export function formatTcfRotationSeed(decision: TcfRotationDecision | null | undefined): string | null {
