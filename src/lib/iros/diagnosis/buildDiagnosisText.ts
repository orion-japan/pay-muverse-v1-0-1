...
// - ここで「今1枚 → 未来1枚 → 診断コア → 返答文」を確定させる
// - meta は “前回値の引き継ぎ用” に留め、診断本文の主決定は flow 2枚引きで行う
// - 2枚のフローは入力非依存の絶対ランダム
// - 診断の核は buildDiagnosisCore を正本として使う
//
// 出力方針：
// - 観測対象：入力そのまま
// - 観測結果：2枚のフローを象徴として比喩化
// - 意識状態：診断コアの state / movement を人間語で返す
// - まとめ：診断コアの tension / decision を返す

import type { DiagnosisMetaLike, DiagnosisSlotLike } from './diagnosisTypes';
import { buildDiagnosisCore } from './buildDiagnosisCore';
import {
  listFlow180,
  parseFlowStateId,
  type FlowStateEntry,
...
function buildAwarenessText(input: {
  coreState: string;
  coreMovement: string;
}): string {
  return [input.coreState, input.coreMovement]
...
function buildSummaryText(input: {
  ...
  const parts = [
    stripTrailingPeriod(input.coreTension),
    stripTrailingPeriod(input.coreDecision),
  ].filter(Boolean);
...
export function buildDiagnosisText(args: {
  ...
  const awarenessText = buildAwarenessText({
    coreState: core.coreState,
    coreMovement: core.coreMovement,
  });

  const summaryText = buildSummaryText({
    coreTension: core.coreTension,
    coreDecision: core.coreDecision,
  });
