...
return blocks;
  ...
  if (isImageFirstEarlyGuard) {
    ...
    if (explicitActionCreateNow) {
      const rcCFinalText = [
        'いまは、一歩を決める前に、戻りたい現実のイメージを少し膨らませる段階です。',
        '',
        '見えている形は、「急いで何かを決める前に、いったん自分の流れを思い出す形」です。',
        '',
        'まず、その流れが戻る入口を一つだけ見つけてください。',
        '',
        '何かを増やすためではなく、自分のペースに戻る場所を思い出すためです。'
      ].join('\\n');
  ...
  const finalPatternContractMsg =
    ...
    return await callWriterLLM({
      ...
      messages: (() => {
        ...
        // HQL_FINAL_MESSAGES_SOVEREIGN_V15B
