            ...
            : flowNextMeaning !== '(none)'
              ...
              const shiftMeaning = (() => {
                ...
                if (continuityKind === 'same_line') {
                  ...
                  if (sk === 'uncover_shift') {
                    return '同じ話の線を保ったまま、いま触れている輪郭だけを狭く残す';
                  }
                ...
                if (continuityKind === 'continuation') {
                  ...
                  if (sk === 'uncover_shift') {
                    return '前の流れを引き継ぎ、いま触れている輪郭だけを狭く残す';
                  }
                ...
                if (sk === 'uncover_shift') {
                  return '表面の説明へ戻さず、いま残っている輪郭だけを狭く残す';
                }
...
const isNormalCompressedPattern =
  ...
  const mergedWriteConstraints = Array.from(
    new Set([
      ...
      ...(isRelationshipReflectionQuestion
        ? [
            '関係性・恋愛相談で、解決/どうしたら/連絡/相手の状態に関わる場合でも、相手側の本心や事情を断定しない',
            '相手側を読みに行きすぎず、連絡が来ないことでユーザー側に何が起きているかを中心に返す',
            'ユーザーの不安が、相手の沈黙や反応をどう重く見せているかを自然に含める',
            '相手の本心や事実を断定しない。「そう映りやすい」「重く見えやすい」という温度で返す',
            '最後は、追う/引く/待つ/距離感/送信提案に落とさず、今こちら側に残っている不安の状態で閉じる',
            '番号・見出しは避け、自然な2段落で返す',
            'このケースでは3〜4文で返す。受け止め→不安の状態→沈黙が重く見える構造までで止める',
        ...
        ? [
            ...
            'OBSは今起きている状態そのものを説明しすぎず、自然な言葉で返す',
            'SHIFTは流れが細くなる一点を、説明しすぎず自然な言葉で返す',
            'NEXTはまだ残っている未解決の一点を、短く自然な言葉にする',
            'SAFEは同じ状態の残りを、読んだ人に伝わる日常語で返す',
            '状態を見ている説明文にしない',
            '案内・観測・解説の言い方にしない',
          ]
        : [
            ...
            // ✅ resonateでは意味展開・理由化・背景構造化へ戻さない
            'resonate時は、同じ意味を別角度で展開しない',
            'resonate時は、説明→補足→余白の構成にしない',
            'resonate時は、理由・背景構造・分岐点を作らない',
            'resonate時は、場に残っている輪郭をそのまま返す',
            'resonate時は、結論ではなく残響を優先する',
            'resonate時は、「〜からに見える」「〜ことがある」「〜ほうが」で逃げない',
            'resonate時は、「〜していい」「〜しなくていい」「〜なくていい」で許可・安心づけにしない',
            'resonate時は、「〜ほうが近い」「〜ほうが合っている」「〜ほうが自然」で評価しない',
            'resonate時は、「〜するといい」「〜持っておくといい」「〜置いておくといい」で助言にしない',
            'resonate時は、「正しいです」「合っています」「近いです」で判定しない',

            '2〜4文で1まとまりにし、少なくとも2まとまり以上で構成する',
            '1つのまとまりに理由・補足・結論を詰め込みすぎない',
            '話題が切り替わる時だけ段落を分ける',
            'OBSは今起きている状態を、自然な言葉で返す',
            'SHIFTは細く残っている輪郭を、自然な言葉で返す',
            'NEXTは未解決の一点を進行指示にせず、そのまま残す',
          ]),
    ])
  );
  ...
  const internalPackForWriter = (() => {
    ...
    const rewritePackWithSeedInstructionCore = (content: string) => {
      ...
      const obsCoreForPack =
        'まず質問への定義・軸を短く返す。emotion_inner / emotion_need が存在しても、OBSの先頭を感情の言い換えだけで開始しない。emotion_inner の実値は本文に丸出しせず、必要な場合だけ後続文の温度調整に使う';
              ...
              const isPartnerSideResonance =
                ...
                if (isPartnerSideResonance) {
                  return [
                    ...
                    '解決や「どうしたら」に関わる場合は、最後に今こちらが取る距離感を日常語で短く伝えてください。',
