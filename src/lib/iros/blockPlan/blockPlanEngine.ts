...
export function detectExplicitBlockPlanTrigger(userText: string): boolean {
  ...
  // 明示トリガーは「出し方・レイアウト指定」だけに絞る
  // - ここでは「構造」「整理」「まとめ」単体は拾わない
  // - それらは内容要求（構造的に見たい）でも普通に出るため
  return /(多段|段で|段落で|段取り|ステップ|step|steps|ブロック|レイアウト|フレーム|枠組み|アウトライン|見出し|セクション|heading|section|テンプレ|テンプレート|型で|章立て|項目で|構成で|段組み|診断を元に|診断をもとに|診断に基づいて|診断にもとづいて|診断を踏まえて|診断ベース|診断から|診断内容|診断結果|さっきの診断|前の診断|この診断|今の診断|深めて|深める|掘り下げ|掘って)/i.test(
    t,
  );
}
...
function detectWantsDeeper(userText: string): boolean {
  ...
  return /(詳しく|詳細|丁寧に|ちゃんと|しっかり|長め|深め|深めて|深める|深掘り|掘り下げ|掘って|背景|理由|本質|診断を元に|診断をもとに|診断に基づいて|診断にもとづいて|診断を踏まえて|診断ベース|診断から|診断内容|診断結果|さっきの診断|前の診断|この診断|今の診断|説明して|解説して|補足して)/i.test(t);
}
...
export function renderBlockPlanSystem4(plan: BlockPlan): string {
  ...
  const bodyDecorRules = [
    `【本文の装飾ルール（積極許可）】`,
    `- 本文では Markdown 装飾を積極的に使ってよい。plain text に戻さない。`,
    `- 箇条書きが有効な箇所では「- 」を優先して使う（例示 / 比較 / 分類 / 手順）。`,
    `- 強調したい語や結論は「**太字**」で明示してよい。各見出しブロックで 1〜3 箇所まで可。`,
    `- 大きな切り替わりでは「---」を 0〜1 回使ってよい。`,
    `- 注意点・定義・補足を短く切り出す時だけ「> 」を使ってよい。`,
    `- 箇条書き・太字・区切り線・引用は、見出しの下で自然に併用してよい。`,
    `- Markdown 記法（## / ** / - / --- / >）は記号のまま出力する。説明文に言い換えない。`,
    `- ただし装飾過多は避け、1ブロックごとに「見出し → 要点 → 必要なら箇条書き」の流れを優先する。`,
  ].join('\n');
