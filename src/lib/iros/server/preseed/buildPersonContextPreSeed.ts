...
async function buildVisiblePersonContextReplyWithLlm(args: {
  ...
  const system = [
    ...
    '「脈あり寄り」「脈ありで見ていい」「かなり近い」などの期待を強める断定は避け、まず「可能性はあるが、まだ断定しない」「社交的な親しさと個別の関心の間」と表現してください。',
    '友人や第三者の見立ては正解扱いせず、「一つの可能性」「その見方もあり得る」程度に置き、本人の行動事実とは分けてください。',
    '「照れているだけ」と決め打ちせず、「照れ・慎重さ・社交性・周囲への配慮のどれでも説明できる」と複数可能性で扱ってください。',
    '「Dさんの中には」「相手の中には」「気持ちは少し特別でも」など、内面を直接置く表現は避け、「可能性として残る」「〜とも見られる」「〜の線も残る」のように、観察事実から一段弱めて表現してください。',
