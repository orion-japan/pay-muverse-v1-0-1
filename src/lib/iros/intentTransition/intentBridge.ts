...
export function decideLaneKey(args: {
  ...
  // ------------------------------------------------
  // B) 明示的な「候補列挙」要求だけ IDEA_BAND
  //    ※ 決断要求（どっちがいい / どうすればいい など）はここに入れない
  // ------------------------------------------------
  const wantsExplicitCandidateList =
    /(候補|選択肢|オプション|案).*(出して|ください|お願い|見せて|教えて|並べて|挙げて)/.test(t) ||
    /(一覧|リスト|一行ずつ|1行ずつ|箇条書き|パターン|候補を[0-9一二三四五六七八九]+(つ|個)?)/.test(t) ||
    /([2-9])\s*(?:つ|個|案)/.test(t);
  ...
  // ------------------------------------------------
  const wantsDecision =
    /(どうすればいい|どうしたらいい|何をしたらいい|何から始める|何から始めたらいい|どっちがいい|どちらがいい|どれがいい|どの方向がいい|どの道がいい|決めきれない|決められない|決めたい|選べない|選びきれない)/.test(
      t,
    ) ||
    /(.+)(か|or|または)(.+)(どっち|どちら|どれ)/.test(t) ||
    /(続けながら動く|環境を変える|辞める|続ける).*(どっち|どちら|どれ|いい)/.test(t);

  if (wantsDecision) return 'T_CONCRETIZE';
