...
export function buildMuMetrics(stats: ConversationStats): MuMetricRecord[] {
  ...
  if (typeof stats.imageFails === 'number' && typeof stats.imageTotal === 'number') {
    out.push(recordImageFailRate(stats.imageFails, stats.imageTotal, stats.conversation_id));
  }
  if (typeof stats.imageFallbacks === 'number' && typeof stats.imageTotal === 'number') {
    out.push(
      recordImageFallbackRate(stats.imageFallbacks, stats.imageTotal, stats.conversation_id),
    );
  }
  ...
  if (typeof stats.fbkPositive === 'number' && typeof stats.fbkTotal === 'number') {
    out.push(recordUserIntentFeedback(stats.fbkPositive, stats.fbkTotal, stats.conversation_id));
  }
