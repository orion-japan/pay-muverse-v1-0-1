...
export default function CreditSummaryPage() {
  ...
  const openGroup = useCallback(async (value: string | null) => {
    if (value == null) return;
    setDrawerOpen(true);
    setDrawerTitle(`${groupBy} = ${value}`);
    setPage(0);
    const res = await withAuth(`/api/admin/credits/summary/users?group_by=${groupBy}&value=${encodeURIComponent(value)}&limit=100&offset=0`);
    const json = await res.json();
    if (res.ok) {
      setList(json.items || []);
      setTotal(json.total || 0);
    } else {
      setList([]); setTotal(0); setMsg(json?.error || json?.detail || "fetch failed");
    }
  }, [groupBy, withAuth]);

  const nextPage = useCallback(async (dir: 1|-1) => {
    const newPage = Math.max(page + dir, 0);
    const value = drawerTitle.split(" = ").pop() || "";
    const offset = newPage * 100;
    const res = await withAuth(`/api/admin/credits/summary/users?group_by=${groupBy}&value=${encodeURIComponent(value)}&limit=100&offset=${offset}`);
    const json = await res.json();
    if (res.ok) { setList(json.items || []); setPage(newPage); }
      ...
      <section className="sum-card">
        ...
        <div className="table-wrap">
          <table className="table">
            ...
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={5} className="muted">なし</td></tr>
              )}
              {rows.map(r => (
                <tr key={String(r.group_value)} onClick={() => openGroup(r.group_value ?? '')} className="clickable">
      ...
      {drawerOpen && (
        <section className="sum-card drawer">
          <div className="row space">
            ...
            <div className="row">
              <button className="btn ghost" onClick={() => nextPage(-1)} disabled={page===0}>←</button>
              <div className="muted">page {page+1}</div>
              <button className="btn ghost" onClick={() => nextPage(1)} disabled={(page+1)*100 >= total}>→</button>
              <button className="btn danger" onClick={() => setDrawerOpen(false)}>閉じる</button>
            </div>
          </div>
          ...
          <div className="table-wrap">
            <table className="table">
              ...
              <tbody>
                {list.length === 0 && <tr><td colSpan={6} className="muted">なし</td></tr>}
                {list.map(u => (
                  <tr key={u.user_code}>
