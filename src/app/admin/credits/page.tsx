...
export default function CreditAdjustPage() {
  ...
  const search = useCallback(async () => {
    ...
    try {
      ...
      if (!u) {
        setSelected(null); setTx([]); setBalance(null);
  ...
  const loadTx = useCallback(async (user_code?: string) => {
    const code = user_code || selected?.user_code;
    if (!code) return;
    try {
      const r1 = await withAuth(`/api/admin/credit-tx?user_code=${encodeURIComponent(code)}&limit=50`);
  ...
  // Enter で検索
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === "Enter") search(); };
  ...
  return (
    <main className="credit-root">
      ...
      <section className="credit-card">
        ...
        <div className="row two">
          <div className="col">
            ...
            <input type="number" className="input" min={1} value={amount}
        ...
        <div className="row gap">
          ...
          <button className="chip" onClick={() => { setAmount(1); setReason("manual_grant"); }}>+1</button>
          <button className="chip" onClick={() => { setAmount(10); setReason("promo10"); }}>+10</button>
          <button className="chip" onClick={() => { setAmount(45); setReason("promo45"); }}>+45</button>
          <button className="chip" onClick={() => { setAmount(90); setReason("promo90"); }}>+90</button>
        </div>
      ...
      <section className="credit-card">
        ...
        <div className="row">
          <input className="input grow" value={voidKey}
        ...
        <div className="table-wrap">
          <table className="table">
            ...
            <tbody>
              {tx.length === 0 && (
                <tr>
                  <td colSpan={4} className="muted">なし</td>
                </tr>
              )}
              {tx.map((t) => (
                <tr key={t.id}>
