...
function withParam(url: string, key: string, value: string) {
  const u = new URL(
    url,
    typeof window !== 'undefined' ? window.location.origin : 'https://zoom.us',
  );
...
export default function KyomeikaiClient() {
  ...
  const canJoinTime = (s: NextSchedule | null) => {
    ...
    const start = d.getTime(),
      end = start + s.duration_min * 60 * 1000;
      ...
      <div className="km-card">
        ...
        <div className="km-actions">
          <button
            ...
            title={
              !canJoinTime(schedule)
                ? '開始10分前から入室できます（出席カウントは開始±10分）'
                : undefined
            }
            type="button"
          >
