...
const STYLE_DESCRIPTIONS: Record<IrosStyle, string> = {
  friendly:
    'やわらかく寄り添う文章。少しくだけた表現もOK。',
  'biz-soft':
    '企業向け。敬語ベースで柔らかく、1on1や企画メモにそのまま使える文章。',
  'biz-formal':
    '会議・資料向け。感情表現を抑え、構造と示唆を中心に整理する文章。',
  plain:
    '装飾少なめのフラットな文章。共感は一言だけ、あとは情報と選択肢の整理に集中。',
};
...
export default function IrosAiSettingsPage() {
  const [style, setStyle] = useState<IrosStyle>('plain');
  ...
  useEffect(() => {
    try {
      ...
      // localStorage 不可の場合はデフォルト(plain)のまま
    } finally {
      setLoaded(true);
    }
  }, []);
  ...
  return (
    <div style={{ padding: '24px' }}>
      <h1 style={{ fontSize: '1.2rem', marginBottom: '12px' }}>Mu 設定</h1>

      <p style={{ opacity: 0.7, marginBottom: '20px', lineHeight: 1.6 }}>
        Mu の「文章スタイル」を切り替えられるようにしました。
        <br />
      ...
      >
        <h2 style={{ fontSize: '1rem', marginBottom: '8px' }}>🗣 文章スタイル</h2>
        <p style={{ fontSize: '0.85rem', opacity: 0.75, marginBottom: '12px' }}>
          いまは <strong>Muverse 内の Mu 全体に対して共通の文章スタイル</strong>{' '}
          として扱います。
        </p>
