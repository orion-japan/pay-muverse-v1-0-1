...
export default function SofiaLogsPage() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      setListLoading(true);
      try {
        const params = new URLSearchParams({ user_code: userCode, page_size: '200' });
  ...
  const creditStats = useMemo(() => {
    ...
    let total = 0;
    let totalAbs = 0;
    const byRole: Record<string, number> = { assistant: 0, user: 0, system: 0, other: 0 };
    const count: Record<string, number> = { assistant: 0, user: 0, system: 0, other: 0 };
  ...
  const downloadCSV = () => {
    ...
    a.href = url;
    a.download = detail?.id ? `sofia_turns_${detail.id}.csv` : 'sofia_turns.csv';
    a.click();
