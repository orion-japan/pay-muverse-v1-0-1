...
export async function POST(req: NextRequest) {
  try {
    ...
    const baseSystem = 'You are Iros. Facts must never be altered. Tone/ordering may be adjusted if hinted.';
    ...
    return NextResponse.json({ ok:false, error: String(e?.message ?? e), stack: e?.stack ?? null }, { status: 500 });
  }
}
