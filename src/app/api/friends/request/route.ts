...
export async function GET(req: NextRequest) {
  ...
  const [incoming, outgoing] = await Promise.all([
    supabase.from('friend_requests').select('*').eq('to_user_code', me).eq('status','pending').order('created_at', { ascending: false }),
    supabase.from('friend_requests').select('*').eq('from_user_code', me).eq('status','pending').order('created_at', { ascending: false }),
  ]);
