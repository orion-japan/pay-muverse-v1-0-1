...
async function writeDiagnosisFromSeed(params: {
  ...
  const writerSystem = [
    ...
    'このケースで「優先順位から消える私」へ安易に寄せないでください。それは相手に放置される文脈のコピーであり、今回の右側ユーザー発言の中心とは限りません。',
    '良いコピー例: 「関係の外に置かれる未来」「もう会えない予定だけが残る」「優先順位から消える約束」「期待していた場所から外される私」。',
    'コピーでは「予感」を多用しないでください。弱く説明的になります。必要なら「優先順位から消えていく私」「大切な場所から外される私」のように、未来の絵がそのまま立ち上がる短い言葉にしてください。',
  ...
  const writerUser = [
    ...
    '右側のユーザー発言が「僕がいなくても」「ほんとに？」「たいしたことしてない」などの場合、そこから「自分の影響が本当に届いたのか」「役に立てたのか」「救えたのかを確かめたい未来」を読んでください。相手の優先順位から消える未来に安易に寄せないでください。',
...
export async function POST(req: NextRequest) {
  try {
    ...
    const system = [
      ...
      'current_future_imaginal と current_future_meaning には、表面の不安だけでなく、その先にある怖さまで入れてください。例: 期待が消える、約束が信じられない、自分は重要ではない、取り残される、関係から外される、もう会えなくなる。',
      ...
      'copy_material には、current_future_imaginal の先にある怖さの素材も入れてください。例: 期待が消える / 取り残される / 重要ではない / 約束の外 / 会えなくなる。',
