...
export async function POST(req: Request) {
  try {
    ...
    if (!user_code || !seed_id) {
      return NextResponse.json({ ok:false, error:'user_code and seed_id/vision_id are required' }, { status:400 });
    }
    ...
    // lanes
    const lanesRaw = Array.isArray(b.lanes) ? b.lanes : (typeof b.lanes === 'string' ? [b.lanes] : []);
    const lanes = lanesRaw.filter((x: any) => typeof x === 'string' && x.trim()).map((s: string) => s.trim());
    ...
    const metric = ((): any | null => {
      ...
      const baseline = Number.isFinite(Number(b.metric.baseline)) ? Number(b.metric.baseline) : undefined;
      const target   = Number.isFinite(Number(b.metric.target))   ? Number(b.metric.target)   : undefined;
      return { type: t, ...(unit ? { unit } : {}), ...(baseline !== undefined ? { baseline } : {}), ...(target !== undefined ? { target } : {}) };
    })();
