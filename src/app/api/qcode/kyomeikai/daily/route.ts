...
export async function POST(req: Request) {
  try {
    ...
    if (!user_code) return NextResponse.json({ ok: false, error: 'user_code required' }, { status: 400 });

    const forDate = String(body.date || '').trim() || new Date(Date.now() + 9*3600*1000).toISOString().slice(0,10);
    ...
    return NextResponse.json({ ok:true, id:data.id, created_at:data.created_at, q: result.q, meta: result.meta });
  } catch (e:any) {
