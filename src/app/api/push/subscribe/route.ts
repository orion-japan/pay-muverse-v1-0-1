...
const isMissingColumn = (msg?: string) =>
  !!msg &&
  /column ["']?enabled["']? does not exist|could not find the ["']enabled["'] column/i.test(msg);
...
async function upsertSubscription(userCode: string, sub: WebPushSubscription, enabled: boolean) {
  // まずは enabled 付きで試す
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { ok: false, message: 'Missing userCode or subscription' },
        { status: 400 },
      );
    }

    const effectiveEnabled = await resolveEffectiveEnabled(userCode, enabled);
    const { error, usedEnabled } = await upsertSubscription(
      userCode,
      subscription,
      effectiveEnabled,
    );
    ...
    return NextResponse.json({
      ok: true,
      note: usedEnabled ? undefined : 'enabled column not found; saved without it',
    });
  } catch (e: any) {
...
export async function PATCH(req: Request) {
  try {
    ...
    if (!userCode || typeof enabled !== 'boolean') {
      return NextResponse.json(
        { ok: false, message: 'userCode and enabled are required' },
        { status: 400 },
      );
    }
    ...
    const qb = supabase
      ...
      .update({ enabled: finalEnabled } as any)
      .eq('user_code', userCode);
