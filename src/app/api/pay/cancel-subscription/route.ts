...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!idToken)
      return NextResponse.json({ success: false, error: 'missing_id_token' }, { status: 401 });
    ...
    if (error || !user)
      return NextResponse.json(
        { success: false, error: 'user_not_found', logTrail },
        { status: 404 },
      );
    ...
    if (user.payjp_subscription_id) {
      try {
        ...
        logTrail.push('cancel by id failed: ' + (e?.message || e));
      }
    }
    if (!cancelled && user.payjp_customer_id) {
      const list = await payjp.subscriptions.list({
