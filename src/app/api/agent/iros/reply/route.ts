              ...
              messages: [
                {
                  role: 'system',
                  content: [
                    'あなたは Mu の RC_STABILIZE 返答を整える担当です。',
                    '保存したことを説明しない。ラベル確認を返さない。対象ラベルを本文に出さない。RC_STABILIZE_SEED から、本人側の揺れ・関係の温度・次に見る基準を返す。surface_reply の3文構成を保持し、最後の判断線を省略しない。',
                    '「〜として扱います」「見ていきます」「受け取ります」「保存しました」は使わない。',
                    '',
                    '目的は、相手の本心を断定せず、相手を見ているようで本人側の不安が動いている状態を、日常語で返すことです。方向・温度・次の見方を本文にしてください。',
                    '',
                    '禁止:',
                    '- 新しい診断を始めない',
                    '- 相手の気持ちを推測しない',
                ...
                },
                {
                  ...
                  content: [
                    ...
                    '',
                    'RC_STABILIZE_SEED をもとに、Muの自然な本文にしてください。surface_reply を短縮しすぎないでください。3文構成を保ち、最後の「返ってくる温度を見る」判断線を必ず残してください。',
                    '禁止：片思いと断定する、相手の本心を決めつける、保存確認だけで終わる、ラベル説明だけで終わる。',
                    '推奨：「見えているのは、相手の気持ちそのものより、分からないまま近づく怖さです。だから今は、答えを急いで確かめるより、軽く話しかけられる距離を作る方が合っています。返しやすい短い言葉を一つ置いて、返ってくる温度を見てください。」のように、対象ラベルで始めず、方向と次の見方を返す。最後の判断線は省略しない。',
                    'RC_STABILIZE_SEED:',
                    `internal_target_label_do_not_say: ${relationshipContextCapture.targetLabel ?? ''}`,
                    `kind: ${relationshipContextCapture.kind ?? ''}`,
                    'concretizeOrigin: RC_STABILIZE',
                    'createMode: stabilize_action',
                    'createReady: false',
                    'primaryAxis: R',
                    'supportAxis: S',
                    'do_not_confirm_storage: true',
                    'do_not_explain_label: true',
                    'do_not_use_target_label_in_body: true',
                    'do_not_start_with_target_label: true',
                    'forbidden_phrases: との関係として / 気になっている相手との関係として / 受け取ります / 見ていきます / 文脈で / として扱います / 相手の気持ちは、いま決めつけない方がいいです / 静かに / 距離を作れるか / 確かめるのが合っています / そっと / そっと確かめる / 返ってくる温度をそっと確かめる',
                    'inner_state: 相手の気持ちを見ているようで、分からないまま近づく怖さが動いている',
                    'relation_state: 関係を確定させる段階ではなく、距離と温度を見る段階',
                    'next_view: 答えを急いで確かめにいかず、軽く話しかけられる距離を作る',
                    'preserve_structure: 3文構成を保つ',
                    'must_keep_final_sentence: 返しやすい短い言葉を一つ置いて、返ってくる温度を見てください。',
                    'do_not_omit: 最後の判断線を省略しない',
                    'surface_reply: 見えているのは、相手の気持ちそのものより、分からないまま近づく怖さです。だから今は、答えを急いで確かめるより、軽く話しかけられる距離を作る方が合っています。返しやすい短い言葉を一つ置いて、返ってくる温度を見てください。',
                  ].join('\\n'),
                },
              ],
            });
...
const metaForRelationshipContextCapture: any = {
    ...
    let preSeedDecision = await resolvePreSeedDecision({
      userText: userTextClean,
      userCode,
      conversationId,
      supabase: supabase as any,
