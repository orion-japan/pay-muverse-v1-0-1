...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!session_id || !file_path) return json({ ok:false, error:"missing session_id/file_path" }, 400);

    const sb = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, { auth:{ persistSession:false }});
    // images 末尾に push
    const { error } = await sb.rpc("jsonb_array_append_obj", {
    ...
    if (error) {
      // フォールバック：現在値を取得して上書き
      const { data: row, error: e1 } = await sb.from("mu_fshot_sessions").select("images").eq("id", session_id).single();
      ...
      const { error: e2 } = await sb.from("mu_fshot_sessions").update({ images: arr, updated_at: new Date().toISOString() }).eq("id", session_id);
