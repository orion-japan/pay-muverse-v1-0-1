...
export default function QDayTimeline({ logs }: { logs: QLog[] }) {
  ...
  return (
    ...
    >
      ...
      {[1, 2, 3, 4, 5].map((v) => {
        ...
        return (
          <g key={v}>
            ...
            <text x={8} y={yy + 4} fontSize="10" fill="#667">
              Q{v}
            </text>
          </g>
        );
      ...
      {/* 折れ線 */}
      {ordered.length > 0 && (
        <polyline points={points} fill="none" stroke="#6f86ff" strokeWidth="2" />
      )}

      {/* 点 */}
      {ordered.map((it, i) => (
        <circle key={i} cx={x(i)} cy={y(it.q_code?.currentQ)} r="3.5" fill="#6f86ff">
