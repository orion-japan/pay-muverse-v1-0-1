      ...
      {rows.map(({ q, total, yin, yang, yinPct, yangPct, color })=>(
        <div key={q} style={{display:'grid', gap:6}}>
          <div style={{display:'flex', justifyContent:'space-between', fontSize:13}}>
            ...
            <span style={{color:'#566'}}>{total}件（陰{yin} / 陽{yang}）</span>
          </div>
          <div style={{
            width:'100%', height:14, borderRadius:7, background:'#eef1f6', overflow:'hidden',
          ...
          <div style={{display:'flex', justifyContent:'space-between', fontSize:12, color:'#667'}}>
            <span>陰 {yinPct}%</span>
            <span>陽 {yangPct}%</span>
          </div>
        </div>
      ))}
      <div style={{marginTop:4, display:'flex', gap:12, alignItems:'center', fontSize:12, color:'#667'}}>
        <span style={{display:'inline-flex', alignItems:'center', gap:6}}>
          <i style={{width:12,height:12,background:'#999',opacity:.6,borderRadius:3,display:'inline-block'}}></i> 陰
        </span>
        <span style={{display:'inline-flex', alignItems:'center', gap:6}}>
          <i style={{width:12,height:12,background:'#999',borderRadius:3,display:'inline-block'}}></i> 陽
        </span>
      </div>
    </div>
  );
