...
type QCode = {
  user_code: string;
  s_ratio: number; r_ratio: number; c_ratio: number; i_ratio: number;
  si_balance: number;
...
export default function QCodeBadge({ userCode }: { userCode: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/qcode/${encodeURIComponent(userCode)}/get`, { cache: 'no-store' });
  ...
  return (
    <div className="qcode-badge" title={`S:${q.s_ratio} R:${q.r_ratio} C:${q.c_ratio} I:${q.i_ratio} / SI:${q.si_balance}`}>
