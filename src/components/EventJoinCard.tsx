...
export default function EventJoinCard({ eventId, title, description }: Props) {
  ...
  const handleJoin = async () => {
    try {
      ...
      if (!res.ok || !json.ok) {
        setStatus('ng')
        setMsg(json.error === 'OUT_OF_WINDOW'
  ...
  return (
    <div className="event-card">
      ...
      <style jsx>{`
        .event-card{border:1px solid #e6e6f0;border-radius:12px;padding:14px;background:#fff}
        .event-title{font-weight:700;margin-bottom:6px}
        .event-desc{font-size:14px;opacity:.8;margin:6px 0 10px}
        .event-btn{padding:10px 14px;border-radius:10px;border:1px solid #d0c4ff;background:#f5f0ff;cursor:pointer}
        .event-btn:disabled{opacity:.6;cursor:not-allowed}
        .event-msg{font-size:12px;color:#a00;margin-top:6px}
      `}</style>
    </div>
  )
}
