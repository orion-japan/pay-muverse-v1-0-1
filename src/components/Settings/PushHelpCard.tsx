  ...
  return (
    <section style={{ border: '1px solid #eee', borderRadius: 12, padding: 16, marginTop: 16 }}>
      ...
      <div style={{ fontSize: 14, lineHeight: 1.6 }}>
        ...
        {onAndroidChrome && (
          <div style={{ marginTop: 8, opacity: 0.8 }}>
            ※ Android Chrome は OS/ブラウザの仕様上、Web
            ページから設定画面へ直接遷移できません。下の「手順を見る」から操作してください。
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 }}>
        ...
        <button onClick={reSubscribe} disabled={!userCode}>
          購読をやり直す
        </button>
        <button onClick={sendTest} disabled={!userCode}>
          テスト通知を送る
        </button>
