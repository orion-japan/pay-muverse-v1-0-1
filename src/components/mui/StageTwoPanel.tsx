...
export default function StageTwoPanel({ user_code }: { user_code?: string }) {
  ...
  const toneBase = useMemo(
    () => ({ phase: 'Mixed', layer18: 'R4', q_current: 'Q3', guardrails: GUARDRAILS }),
    [],
  );
    ...
    <div className="mui-stage2">
      <header className="head">
        <div>
          ...
          <p className="muted">
            OCR_ID: <code>{ocrId}</code> ／ 会話ID: <code>{convId}</code> ／ プラン:{' '}
            <strong>{paid ? '有効' : '未購入'}</strong>
          </p>
        </div>
      ...
      {step === 1 && (
        <StepCard
          title={PROMPTS_STAGE2.step1.title}
          footer={
            <div className="actions">
              <button
                ...
                onClick={() =>
                  requirePaid(async () => {
                    await persist('stage2-1', PROMPTS_STAGE2.step1.nextStep);
                  })
                }
              >
                この内容を記録
      ...
      {step === 2 && (
        <StepCard
          title={PROMPTS_STAGE2.step2.title}
          footer={
            <div className="actions">
              <button
                className="btn"
                ...
                onClick={() =>
                  requirePaid(async () => {
                    await persist('stage2-2', PROMPTS_STAGE2.step2.nextStep);
                  })
                }
              >
                この内容を記録
      ...
      {step === 3 && (
        <StepCard
          title={PROMPTS_STAGE2.step3.title}
          footer={
            <div className="actions">
              <button
                className="btn"
                ...
                onClick={() =>
                  requirePaid(async () => {
                    await persist('stage2-3', PROMPTS_STAGE2.step3.nextStep);
                  })
                }
              >
                この内容を記録
      ...
      <style jsx global>{`
        ...
        .badge {
          ...
          border-radius: 999px;
          font-size: 12px;
          box-shadow: 0 2px 8px rgba(129, 103, 255, 0.15);
        }
        .flash {
        ...
        .card {
          ...
          margin-bottom: 14px;
          box-shadow:
            0 6px 24px rgba(129, 103, 255, 0.12),
            0 2px 8px rgba(255, 123, 212, 0.1);
        }
        .card__title {
          font-weight: 800;
        ...
        .prompt {
          ...
          border: 1px dashed rgba(129, 103, 255, 0.35);
          border-radius: 12px;
          padding: 10px;
        }
        .actions {
        ...
        .btn:hover {
          background: #fafaff;
          box-shadow: 0 6px 18px rgba(129, 103, 255, 0.18);
        ...
        .btn--primary {
          ...
          border-color: rgba(129, 103, 255, 0.35);
          color: #2a234d;
          font-weight: 800;
        }
        .chatbox {
