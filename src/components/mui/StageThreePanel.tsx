...
次の一歩：境界/トリガー/二次感情をそれぞれ1語ずつ書く。`,
  ...
  step2: {
    title: '② 事例から実践',
    body: `似たケース（友人・ネットの事例・過去の自分）を1つ取り上げ、成功した最小の一歩を真似できる形で具体化します（20〜60字）。

次の一歩：成功事例の「最小の一歩」を1つだけ書く。`,
    nextStep: '最小の一歩を1つだけ言語化',
  },
  step3: {
    title: '③ 共鳴パターンを知る',
...
export default function StageThreePanel({ user_code }: { user_code?: string }) {
  ...
  useEffect(() => {
    setOcrId(getOrCreateOcrId());
  }, []);
  useEffect(() => {
    if (ocrId) setConvId(getOrCreateConvId(ocrId));
  }, [ocrId]);
          ...
          footer={
            <div className="actions">
              <button
                ...
                disabled={busy}
                onClick={async () => {
                  await persist('stage3-1', PROMPTS_STAGE3.step1.nextStep);
                }}
              >
                この内容を記録
              </button>
          ...
          footer={
            <div className="actions">
              <button
                ...
                disabled={busy}
                onClick={async () => {
                  await persist('stage3-2', PROMPTS_STAGE3.step2.nextStep);
                }}
              >
                この内容を記録
              </button>
          ...
          footer={
            <div className="actions">
              <button
                ...
                disabled={busy}
                onClick={async () => {
                  await persist('stage3-3', PROMPTS_STAGE3.step3.nextStep);
                }}
              >
                この内容を記録
              </button>
