...
export const STAGE1_CONTENT: Record<'stage1-1'|'stage1-2'|'stage1-3', StageContentEntry> = {
  'stage1-1': {
    title: '① 状況と状態',
    uiText:
`【Irosガード】断定禁止 / 選択肢は2つ / 行動は1つ

相手の文脈（頻度・返答間隔・語尾トーン）から関係温度を仮に言葉にします。
次の一歩：『事実』と『解釈』を1行ずつ分けて書く。`,
    sysPrompt:
`あなたは日本語コーチ。入力の会話文から「事実」と「解釈」を分け、関係温度を低/中/高のどれかで仮説提示。
...
`{
  ...
  'stage1-2': {
    title: '② パターンの解説',
    uiText:
`7つの歪みパターン（依存/干渉/逃避/支配/投影/置換/昇華）の兆候を指標に、最も近いものを仮置き。
次の一歩：当てはまると思うパターンを最大1つ選ぶ。`,
    sysPrompt:
`会話文から兆候を抽出し、次の keys のいずれか1つを選ぶ: ${MUI_PATTERNS.map(x=>x.key).join(' / ')}。
...
export const STAGE2_CONTENT: Record<'stage2-1'|'stage2-2'|'stage2-3', StageContentEntry> = {
  'stage2-1': {
    title: '① パターンから相手の状態',
    uiText:
`選んだパターンを手掛かりに、相手の内的ニーズを仮説として1行で言語化。
次の一歩：ニーズを尊重する前置きの1文を書く。`,
    sysPrompt:
`パターンと会話文から、相手の内的ニーズを1行で仮説化。評価語禁止。
出力: { "need": "…" , "preface":"ニーズ尊重の前置き1文" }`,
  ...
  'stage2-2': {
    title: '② 返信の方法',
    uiText:
`Irosトーンで使える返信テンプレを2択まで提示。
次の一歩：どちらか1つを選んで下書きへ反映。`,
    sysPrompt:
`会話の流れと選択パターンに合うテンプレを ${REPLY_TEMPLATES.length} 個から最大2つ選び、キーだけ返す。
出力: { "suggest": ["安心系","境界系"] }`,
  ...
  'stage2-3': {
    title: '③ パターンの対処法',
    uiText:
`関係温度を下げずに進める最小の対処行動を1つに絞る（24h以内/可逆/相手の尊厳）。
次の一歩：実行タイミング（いつ・どの場面）を決める。`,
    sysPrompt:
`入力内容から「最小の具体行動」を1つだけ提案。24h以内/可逆/短時間。
...
export const STAGE3_CONTENT: Record<'stage3-1'|'stage3-2'|'stage3-3', StageContentEntry> = {
  'stage3-1': {
    title: '① 相手のパターンを深掘り',
    uiText:
`第二段階の仮説パターンについて、境界（どこから不快/安心）・トリガー（何で反応）・二次感情（本音の奥）を1語ずつ。`,
    sysPrompt:
`会話から「境界/トリガー/二次感情」を各1語で抽出。出力はJSONのみ。
{ "boundary":"…", "trigger":"…", "secondary_emotion":"…" }`,
  ...
  'stage3-2': {
    title: '② 事例から実践',
    uiText:
`似た会話事例の成功例を1つ引用→自分向けに言い換え（合意→要望→選択肢）。`,
    sysPrompt:
`(1) 類似会話の一文サンプルを1つ (2) それをユーザー向けに3文テンプレへ言い換え。
出力: { sample:"…", rephrase:{ agree:"…", ask:"…", choices:["…","…"] } }`,
  ...
  'stage3-3': {
    title: '③ 共鳴パターンを知る',
    uiText:
`相手が反応しやすい言い方（共鳴パターン）をラベル化：安心/境界/具体化/軽さ/時間差の5類。`,
    sysPrompt:
`テキストから「共鳴パターン」を1～2個選ぶ: ["安心","境界","具体化","軽さ","時間差"]。
...
export const STAGE4_CONTENT: Record<'stage4-1'|'stage4-2'|'stage4-3', StageContentEntry> = {
  'stage4-1': {
    title: '① 自分と相手の共鳴ポイント',
    uiText:
`第三段階までで見えた “響きやすい言い回し/価値観/タイミング” を1文に集約。`,
    sysPrompt:
`入力のヒントから共鳴ポイントを1文で作る。「〜が大事」「〜だと落ち着く」の形。
出力: { "point":"…"} `,
  ...
  'stage4-2': {
    title: '② 共鳴パターン',
    uiText:
`合意→要望→選択肢（2択）の3文を、共鳴ポイントに合わせて微調整して完成。`,
    sysPrompt:
`共鳴ポイントに沿って3文テンプレを最終化。短文/やさしい敬体。
出力: { agree:"…", ask:"…", choices:["…","…"] }`,
  ...
  'stage4-3': {
    title: '③ 愛の育み方',
    uiText:
`24h以内・可逆・短時間の「最小の育み行動」を1つだけ決める。`,
    sysPrompt:
`次の候補から1つだけ提案し、実行条件(いつ/どの場面)を1行で。候補: ${MICRO_EXPERIMENTS_24H.join(' / ')}
出力: { "action":"…", "when":"…" }`,
    outputFormat:`{ action: string, when: string }`,
...
export const STAGE_CONTENT_ALL = {
  ...STAGE1_CONTENT, ...STAGE2_CONTENT, ...STAGE3_CONTENT, ...STAGE4_CONTENT,
...
export function toQCode(currentQ: QCode, depthStage: string, distribution?: Partial<Record<QCode, number>>) {
  return distribution
    ? { currentQ, depthStage, distribution }
    : { currentQ, depthStage };
}
...
export function stageGuardrails(subId: MuiSubId): ToneGuard[] {
  ...
  switch (coarseSubId(subId)) {
    case 'stage1': return ['断定禁止','選択肢は2つ','行動は1つ'];
    case 'stage2': return ['断定禁止','選択肢は2つ','行動は1つ'];
    case 'stage3': return ['断定禁止','選択肢は2つ','行動は1つ'];
    case 'stage4': return ['断定禁止','選択肢は2つ','行動は1つ'];
  }
}
