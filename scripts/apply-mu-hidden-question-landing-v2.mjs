...
// まず v1 自体を LF に正規化し、read() を LF 正規化する形へ補正してから実行する。
source = source.replace(/\r\n/g, '\n');

const readPattern = /function read\(rel\) \{\n\s*return fs\.readFileSync\(abs\(rel\), 'utf8'\);\n\}/m;
const fixedRead = `function read(rel) {
...
if (source.includes("replace(/\\r\\n/g, '\\n')")) {
  ...
  source = source.replace(readPattern, fixedRead);
  fs.writeFileSync(patcherPath, source, 'utf8');
  console.log('[fixed] v1 patcher read() now normalizes CRLF');
} else {
